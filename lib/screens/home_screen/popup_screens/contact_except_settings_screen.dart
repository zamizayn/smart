...
class ContactExceptSettingsScreen extends StatefulWidget {

  //const ContactExceptSettingsScreen({Key? key}) : super(key: key);
  String type;
  ContactExceptSettingsScreen({Key? key,required this.type}) : super(key: key);


  @override
  State<ContactExceptSettingsScreen> createState() => _ContactExceptSettingsScreenState();
}

class _ContactExceptSettingsScreenState extends State<ContactExceptSettingsScreen> {
  ...
  void initState() {
   ...
   // privacy.blockedStatusUsers.clear();
    ...
    for (var user in privacy.blockedStatusUsers) {
      if(user['excepted_user_status']){
        selectedIndices.add(int.parse(user['user_id']));
      }
    }
    count=selectedIndices.length;
  //selectedIndices = privacy.blockedUsersAbout;
  //print("log " + selectedIndices.toString());*/
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        //  user.data.length.toString() + " contacts",
                        (count==1)?
                        '1 contact excluded': '$count contacts excluded',
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      ...
                      setState(() {
                        ...
                        if (selectAll) {
                          selectedIndices = Set<int>.from(
                            List.generate(privacy.blockedStatusUsers.length, (index) => int.parse(privacy.blockedStatusUsers[index]['user_id'])),
                          );
                        } else {
                          selectedIndices.clear();
                        }
                ...
                Padding(
                  ...
                  ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                     ...
                     bool isChecked = selectedIndices.contains(int.parse(privacy.blockedStatusUsers[index]['user_id']));
                    // bool isChecked = privacy.blockedStatusUsers[index]['excepted_user_status']?true:false;
                     print('isChecked : ');
                     print(selectedIndices);
                     print(privacy.blockedStatusUsers[index]['user_id'].toString());
                      print(privacy.blockedStatusUsers[index]['excepted_user_status']);
                     ...
                     for (var element in phoneContacts) {
                       element.phones?.forEach((phone) {
                         ...
                         if (privacy.blockedStatusUsers[index]['phone'].replaceAll(' ', '').replaceAll('+', '') == phone.value!.replaceAll(' ', '').replaceAll('+', '')) {
                           displayName = element.displayName;
                           print(displayName);
                           privacy.blockedStatusUsers[index]['name']=element.displayName;
                         }
                       });
                     }
                      ...
                      return Container(
                        ...
                        child: InkWell(
                          onTap: (){
                            setState(() {
                              if (isChecked) {
                                selectedIndices.remove(int.parse(privacy.blockedStatusUsers[index]['user_id']));
                                isChecked = false;
                              } else {
                                selectedIndices.add(int.parse(privacy.blockedStatusUsers[index]['user_id']));
                                isChecked = true;
                              }
                              count = selectedIndices.length;
                            });
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(15)),
                                child: CircleAvatar(
                                  backgroundImage: privacy.blockedStatusUsers[index]['profile_pic'] !=
                                      null
                                      ? NetworkImage(
                                      privacy.blockedStatusUsers[index]['profile_pic'])
                                      : const NetworkImage(
                                      'https://cdn.pixabay.com/photo/2016/08/08/09/17/avatar-1577909_960_720.png'),
                                ),
                              ),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                     ...
                                     // displayName ?? privacy.blockedStatusUsers[index]['name'],
                                      style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        color: Colors.grey[700],
                                      ),
                                ...
                                child: Checkbox(
                                  ...
                                  onChanged: (bool? value) {
                                    setState(() {
                                      ...
                                      isChecked = selectedIndices.contains(int.parse(privacy.blockedStatusUsers[index]['user_id'])); // Update isChecked based on new selectedIndices list
                                      count = selectedIndices.length;
                                    });
                                  },
            ...
            floatingActionButton: FloatingActionButton(
              ...
              onPressed: () {
                ...
                switch(widget.type) {
                  case 'lastseen': {
                    (selectedIndices.isEmpty)?
                    privacy.setLastseenStatus(1, selectedIndices.toString(),'except', context):
                    privacy.setLastseenStatus(
                  ...
                  case 'profile': {
                    (selectedIndices.isEmpty)?
                    privacy.setProfileStatus(1, selectedIndices.toString(), 'except',context):
                    privacy.setProfileStatus(
                        2, selected.replaceAll('}', ''),'except', context);
                  }
                  break;
                  ...
                  case 'group': {
                    print('llll');
                    (selectedIndices.isEmpty)?
                    privacy.setGroupStatus(1, selectedIndices.toString(),'except', context):
                    privacy.setGroupStatus(
                        2, selected.replaceAll('}', ''),'except', context);
                  }
                  break;
